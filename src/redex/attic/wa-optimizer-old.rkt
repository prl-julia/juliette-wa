...
(test-equal (apply-reduction-relation* ->opt-expr (term (evalt ,mt1-1 ,callf1)))
            (term ((evalt ,mt1-1 "f-int"))))
; (| x ; f(1) |)_{f(x::Int)="f-int"} ~~> (| "f-int" |)_{f(x::Int)="f-int"}
(test-equal (apply-reduction-relation* ->opt-expr (term (evalt ,mt1-1 (seq x ,callf1))))
            (term ((evalt ,mt1-1 (seq x "f-int")))))
; (| k() + k() |)_{k() = 2} ~~> (| 4 |)_{k() = 2}
(test-equal (apply-reduction-relation* ->opt-expr (term (evalt ,mt2-1 (pcall + ,callk ,callk))))
            (term ((evalt ,mt2-1 4))))
...
; {f(x) = x + k(), k() = 2, g() = k() + 5, f(x) = x + 2, g() = 7, h(x) = f(x) + 7}
(test-predicate (alpha-eq? (apply-reduction-relation* ->opt-tbl mt2-5))
                (term (((mdef "h" ((:: x ,tTop)) (pcall + ,callfx 7)) • ((mdef "g" () 7) • (,ftop-plusx2 • ,mt2-5))))))
...
(test-predicate (alpha-eq? (apply-reduction-relation* ->opt-tbl mt2-6))
                (term (((mdef "g" () 7) • ((mdef "h" ((:: x ,tTop)) (pcall + ,callfx 7)) • (,ftop-plusx2 • ,mt2-6))))))
