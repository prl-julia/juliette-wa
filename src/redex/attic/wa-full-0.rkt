...
;; Returns the list of only the latest methods
...
;; Returns the list of all MT methods applicable to (σ ...)
;; if equivalent methods are not already in (md_acc ...)
;; Accumulator: distinct methods applicable to (σ ...)
;;              found at the already-processed part of MT
(define-metafunction WA-full
  get-applicable-methods/acc : (md_acc ...) MT mname (σ ...) -> (md ...)
  ; empty table -- everything is in the accumulator
  [(get-applicable-methods/acc (md_acc ...) ∅ _ _)
     (md_acc ...)]
  ; md is applicable and an equivalent is not in (md_acc ...)
  [(get-applicable-methods/acc (md_acc ...) (md • MT) mname (σ ...))
     ; call recursively with updated accumulator
     (get-applicable-methods/acc (md md_acc ...) MT mname (σ ...))
     ; get details of the next method definition
     (where (mdef mname ((:: x τ) ...) e) md)
     ; check if the method is applicable (tags are subtypes of its annotations)
     (side-condition (judgment-holds (<:-tuple (σ ...) (τ ...))))
     ; make sure the accumulator does not already contain an equivalent method
     ; (because the accumulator should contain only the newest methods
     ;  if there are multiple equivalent ones in MT)
     (where #f (contains-equiv-md (md_acc ...) md))]
  ; otherwise (if md is not applicable or already accounted for) skip md
  [(get-applicable-methods/acc (md_acc ...) (md • MT) mname (σ ...))
     (get-applicable-methods/acc (md_acc ...)    MT mname (σ ...))]
)
...
;; Returns the list of MT methods applicable to mname(σ ...)
;; If there are multiple equivalent definitions of the same method,
;;   only the newest are returned
(define-metafunction WA-full
  get-applicable-methods : MT mname (σ ...) -> (md ...)
  [(get-applicable-methods MT mname (σ ...))
   (get-applicable-methods/acc () MT mname (σ ...))]
)
...
(define-metafunction WA-full
  ...
  [(getmd MT mname (σ ...))
   ; minimal of the applicable methods
   (min-method (md ...))
